...
const SearchBar: FC<SearchBarProps> = ({ showDropDown, handleChange, searchInput, display, searchList }) => {
    console.log("search");
    ...
    return (
        ...
        <div className="relative flex flex-row place-content-center">
          <input
            type="text"
            onBlur={() => setTimeout(showDropDown, 50)}
          ...
          {display && (
           <div className="flex flex-row flex-wrap absolute top-12 w-full h-fit bg-black border mx-auto rounded-b-lg p-2 lg:w-[28rem]">
           {searchList && searchList.map((data, id) => {
             // Move the logic outside of JSX
             const encodedData = encodeURIComponent(data);
             return (
               <Link to={"/search/" + encodedData}
                 state={data}
                 key={id}
